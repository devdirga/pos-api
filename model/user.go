...
func CreateUser(c echo.Context) error {
	...
	// enc := helper.EncryptString(password, configuration.EncryptionKey)

	user1 := helper.User{Name: name, Email: email, Password: password}
...
func ValidateUser(email, password string, c echo.Context) (bool, error) {
	...
	decrypted := helper.DecryptString(user1.Password, configuration.EncryptionKey)
...
func GetUser(c echo.Context) (err error) {
	var user1 []helper.User
	result := helper.GormDB.Find(&user1)
	// return helper.CustomHTTPSuccess{}
...
func Login(c echo.Context) error {
	...
	configuration := helper.GetConfig()
	decrypted := user1.Password //helper.DecryptString(user1.Password, configuration.EncryptionKey)
